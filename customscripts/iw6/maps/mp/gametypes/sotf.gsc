...
main()
{
	...
	// For teambased games, see if this is the first crate dropped in the game
...
initializeMatchRules()
{
	...
	registerRoundLimitDvar( "sotf", 1 );		
	SetDynamicDvar( "scr_sotf_winlimit", 1 );
	registerWinLimitDvar( "sotf", 1 );	
	SetDynamicDvar( "scr_sotf_halftime", 0 );
	registerHalfTimeDvar( "sotf", 0 );			
...
startSpawnChest()
{
	...
	// Private Match Rules
	crateAmount = GetDvarInt( "scr_sotf_crateamount", 1 );
	crateTimer = GetDvarInt( "scr_sotf_cratetimer", 30 );
...
sotfCrateThink( dropType )
{
	...
	crateUseCount = 0;
	crateRemainingUses = GetDvarInt( "scr_sotf_crategunamount", 6 );
	...
	{	
		...
		// Team based version allows you to grab 6 weapons before it disappears
