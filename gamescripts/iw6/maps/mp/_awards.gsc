...
onPlayerConnect()
{
	for(;;)
	{
		...
		{
			player setCommonPlayerData( "round", "awardCount", 0 );
...
incPlayerRecord( ref )
{
	...
	self setCommonPlayerData( "awards", ref, recordValue + 1 );
...
assignAwards()
{
	...
	foreach ( player in level.players )
	{
		...
		{
			//println( "Aborting awards, non ranked match" );
			return;
		}
	...
	if ( !matchMakingGame() )
	{
		foreach ( player in level.players )
		{
			awardCount = player getCommonPlayerData( "round", "awardCount" );
			if ( awardCount == 0 )
...
giveAward( ref )
{
	awardCount = self getCommonPlayerData( "round", "awardCount" );
	...
	if ( awardCount < 5 )
	{	
		self setCommonPlayerData( "round", "awards", awardCount, "award", ref );
		self setCommonPlayerData( "round", "awards", awardCount, "value", value );
	}
	
	awardCount++;
	self SetCommonPlayerData( "round", "awardCount", awardCount );
...
monitorSwaps()
{
	...
	for ( ;; )
	{
		...
		if ( !IsDefined( self.usedWeapons[ weapon ] ) )
		{
			self.usedWeapons[ weapon ] = true;
...
monitorPositionCamping()
{
	...
	CAMPTHRESHOLD = 512;
	CAMPTHRESHOLD_SQ = squared(CAMPTHRESHOLD);
	...
	{	
		...
		{
			if ( DistanceSquared( self.positionArray[0], self.origin ) < CAMPTHRESHOLD_SQ && DistanceSquared( self.positionArray[1], self.positionArray[0] ) < CAMPTHRESHOLD_SQ )
			{
