...
//	SetDvar( "ui_remoteTankUseTime", level.tankSettings[ "remote_tank" ].timeOut );
...
removeWeapons()
{
	...
	//	we need to get the primary list again here instead of using self.weaponlist because it isn't reliable
	//	there was a bug where you could pick up a weapon while pulling out the killstreak and then you'd end up with 3+ weapons when you were done, getting the primary list instead of self.weaponlist fixes that
	//	this is only necessary for the tank because of the way we need to remove weapons and give you the laptop when you activate the killstreak (remote turret, sentry, ims, sam don't need this fix)
	...
	foreach( weapon in weaponlist )
	{
		...
		// keep a list of the weapons we take because weaponlist isn't reliable
...
tank_handleDisconnect()
{
	...
	// make sure to tell the turret it has died so it ends threads
...
//	vehicle.owner SetPlayerData( "ugvDamaged", 1 );
	...
	if( IsDefined( weapon ) )
	{
		...
		case "emp_grenade_mp":
			// it doesn't completely get destroyed but we'll take ammo away so it can't shoot for a second and we'll up the damage state for static on the screen constantly
			modifiedDamage = 0;
...
tank_EMPGrenaded() // sellf == vehicle
{
...
//	numBullets = self.owner GetPlayerData( "ugvBullets" );
...
//	self.owner SetPlayerData( "ugvBullets", numBullets );
