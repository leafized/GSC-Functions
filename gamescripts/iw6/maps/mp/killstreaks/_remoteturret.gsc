...
tryUseRemoteMGTurret( lifeId, streakName )
{
...
startUsingRemoteTurret() // self == turret
{
	...
	owner setLowerMessage( "early_exit", level.turretSettings[ self.turretType ].hintExit, undefined, undefined, undefined, undefined, undefined, undefined, true );
}
...
watchEnterAndExit() // self == turret
{
	...
	while( true )
	{
		...
		// if we get emp grenaded, don't show the message
		if( IsDefined( owner.empGrenaded ) && owner.empGrenaded )
		{
			if( !IsDefined( owner.enter_message_deleted ) || !owner.enter_message_deleted )
			{
				owner.enter_message_deleted = true;
		...
		// if we are linked to another entity, don't show the message
		if( owner IsLinked() && !owner.using_remote_turret )
		{
			if( !IsDefined( owner.enter_message_deleted ) || !owner.enter_message_deleted )
			{
				owner.enter_message_deleted = true;
