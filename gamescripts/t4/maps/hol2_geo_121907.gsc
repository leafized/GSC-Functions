...
betty_think( trigger )
{
	...
	for( i = 0; i < players.size; i++ )
	{
		...
		else if( Distance( player.origin, trigger.origin ) > trigger.radius + ( trigger.radius * radiusMultiplier ) )
		{
	...
	for( i = 0; i < level.betty_trigs.size; i++ )
	{
		...
		if( Distance2D( trigger.origin, otherBetty.origin ) <= trigger.radius + ( trigger.radius * radiusMultiplier ) )
		{
			// set it off!
