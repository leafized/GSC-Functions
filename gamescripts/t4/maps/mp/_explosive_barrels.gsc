...
oil_spill_burn_after()
{
	...
	if ( !isdefined( self.damageOwner ) ) 
		self radiusdamage (self.origin, 4, 10, 10, undefined, "MOD_EXPLOSIVE", "explodable_barrel"  );
	else
		self radiusdamage (self.origin, 4, 10, 10, self.damageOwner, "MOD_EXPLOSIVE", "explodable_barrel" );
}
...
oil_spill_burn(P, dest)
{
	...
	while(1)
	{
		...
		for(i=0; i<barrels.size; i++)
		{
			...
			if(distancesquared(P, pos) < distsqr) 
			{
				...
				if ( !isdefined ( self.damageOwner ) )
					self radiusdamage (barrels[i].origin, 4, d, d, undefined, "MOD_EXPLOSIVE", "explodable_barrel" );
				else
					self radiusdamage (barrels[i].origin, 4, d, d, self.damageOwner, "MOD_EXPLOSIVE", "explodable_barrel" );
	...
	if( distance(P, self.start.origin) < 32)
	{
		d = (80 + randomfloat(10));
		if ( !isdefined ( self.damageOwner ) )
			self radiusdamage (self.barrel.origin, 4, d, d, undefined, "MOD_EXPLOSIVE", "explodable_barrel" );
		else
			self radiusdamage (self.barrel.origin, 4, d, d, self.damageOwner, "MOD_EXPLOSIVE", "explodable_barrel" );
	}
}
...
oil_spill_burn_section(P)
{
	...
	{
		if ( !isdefined ( self.damageOwner ) )
			self radiusdamage(P, 32, 5, 1, undefined, "MOD_EXPLOSIVE", "explodable_barrel" );
		else
			self radiusdamage(P, 32, 5, 1, self.damageOwner, "MOD_EXPLOSIVE", "explodable_barrel" );
		time += 1;
		wait 1;
	}
}
...
explodable_barrel_explode()
{
	...
	// do not pass damage owner if they have disconnected before the barrels explode
	if ( !isdefined( self.damageOwner ) ) 
		self radiusDamage(self.origin + (0,0,30), blastRadius, maxDamage, minDamage, undefined, "MOD_EXPLOSIVE", "explodable_barrel" );
	else
