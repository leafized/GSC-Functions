...
treasure_chest_init()
{
	...
	if (level.chests.size > 1)
	{
		...
		if(level.script == "nazi_zombie_kyassuru")
		{
...
show_magic_box()
{
	...
	if(isDefined(level.script) && level.script != "nazi_zombie_kyassuru")	
	{
...
{	
	...
	else
	{
		...
		if(level.script != "nazi_zombie_prototype")
		{
			...
			if( isDefined(level.script) && level.script == "nazi_zombie_kyassuru" && level.box_moved == true && isDefined(level.pulls_since_last_ray_gun) )
			{
				level.pulls_since_last_ray_gun += 1;
			}
			
			if( isDefined(level.script) && level.script == "nazi_zombie_kyassuru" && isDefined(level.pulls_since_last_tesla_gun) )
			{				
...
treasure_chest_move(lid)
{
	...
	else if(level.script != "nazi_zombie_kyassuru")
	{
	...
	while(level.verify_chest == false && (level.script == "nazi_zombie_asylum" || level.script == "nazi_zombie_factory" || level.script == "nazi_zombie_kyassuru"))
	{
		level.chest_index++;
		if (level.chest_index >= level.chests.size)
		{
		...
		while(level.script != "nazi_zombie_kyassuru" && level.script != "nazi_zombie_factory" && level.chests[level.chest_index].origin == level.chests[old_chest_index].origin)
		{	
			level.chest_index++;
		}
...
weapon_spawn_think()
{
	...
	for( ;; )
	{
		...
		if( !player_has_weapon )
		{
			...
			{
				play_sound_on_ent( "no_purchase" );
				player thread maps\nazi_zombie_kyassuru_blockers::play_no_money_purchase_dialog();
