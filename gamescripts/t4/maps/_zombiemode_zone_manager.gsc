...
manage_zones( initial_zone )
{
	...
	while(getdvarint("noclip") == 0 ||getdvarint("notarget") != 0	)
	{
		...
		// Changed from forcing "reciever_zone" into user defind one.
		if ( !a_zone_is_active )
		{
			if( isDefined( level.DLC3.initialZones[ 0 ] ) )
			{
				level.zones[ level.DLC3.initialZones[ 0 ] ].is_active = true;
				level.zones[ level.DLC3.initialZones[ 0 ] ].is_occupied = true;
			}
		}
