...
main()
{
	// MikeD: Do not enable until stable.
...
setup_player_stats()
{
	...
	// Hits, Used for calculating location damage percent.
...
//	init_stat( "level", "none" ); 
	...
	// career Location Damage
//	init_stat( "career", "none" ); 
	...
	// TEMP WEAPONS TILL WE GET COD5 WEAPONS IN
...
init_weapon_stat( weapon_name )
{
	...
	// Add weaponname to a list so we can print it out easier later.
...
set_weapon_stat( weapon_name, stat_name, value, career_value )
{
	...
	// Set the dvar( TEMP FOR MENUS, WE MAY WANT A BETTER METHOD )
...
{
	// Disables the current gun and stores the data.
	...
	// Use an int to determine which is the favorite.
	...
	// Now from the int, determine the weapon name.
...
init_stat( stat_group, stat_name, value )
{
	...
	// If the stat_group is career, then carry over the previous stat info.
...
// Sets a the given stat to a dvar.
store_dvar( stat_group, stat_name )
{
...
// Get the value of the given stat dvar
...
//		FPrintLn( file, tab + result_text + self._stats["track_player"][i]["position"] + ", " + self._stats["track_player"][i]["time"] + " ); " ); 
	}
...
add_track_player_results( pos, time, dead_enemy_pos, killed_pos )
{
	...
	// Set the color and text of the position
	color = ( 1, 1, 1 ); 
	string = "."; 
	scale = 1; 
...
draw_track_pos( position, msg, scale, color, time, time_color, position2, dead_enemy_pos, killed_pos )
{
	...
	{
		players = get_players();
		player_distsqrd = DistanceSquared( players[0].origin, position );
