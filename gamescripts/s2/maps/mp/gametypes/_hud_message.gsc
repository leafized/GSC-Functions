 ...
 * Decompiled File: _hud_message.gsc
 * Game: Call of Duty: WWII
 * Platform: PC
 * Function Count: 44
 * Decompile Time: 2071 ms
 * Timestamp: 8/24/2021 10:22:27 PM
*******************************************************************/
...
{
	var_06 = spawnstruct();
	var_06.var_9A2E = param_00;//titleText
	var_06.var_6811 = param_01;//notifyText
	var_06.var_5019 = param_02;//iconName
	var_06.var_00AB = param_03;//glowColor (depreciated)
	var_06.var_8F2C = param_04;//sound
	var_06.var_3511 = param_05;//duration
	func_680B(var_06);
}
...
//Function Number: 6
//notifyMessage
func_680B(param_00 /*notifyData*/)
{
	self endon("death");
	self endon("disconnect");
	if(!isdefined(param_00.var_8CA3))
	{
		param_00.var_8CA3 = 0;//notifyData.slot = 0
	}

	var_01 = param_00.var_8CA3;
	if(!isdefined(param_00.var_01B9))//notifyData.type
	{
		param_00.var_01B9 = "";
	}

	if(!isdefined(self.var_3202[var_01]))//self.doingSplash
	{
		thread func_8C0F(param_00);//showNotifyMessage
		return;
	}

	self.var_9106[var_01][self.var_9106[var_01].size] = param_00;//splashQueue
}
...
func_8C0F(param_00)
{
	...
	if(level.var_3F9D)
	{
		if(isdefined(param_00.var_01B9) && param_00.var_01B9 == "rank")//param_00 = notifyData
		{
			self setclientdvar("ui_promotion",1);
			self.var_75E5 = 1;//self.postGamePromotion
		}
	...
	self.var_3202[var_01] = param_00;//self.doingSplas[slot] = notifyData
	thread func_7D65();
	func_A6F9(0);//waitRequireVisibility
	...
	if(isdefined(param_00.var_8F2C))//var_8F2C - playLocalSound
	{
	...
	{
		maps\mp\_utility::func_5C43(param_00.var_5C49);//leaderDialogOnPlayer
	}

	var_03 = param_00.var_00AB;
	var_04 = self.var_6813;
	if(isdefined(param_00.var_9A2E))//notifyData.titleText
	{
		if(isdefined(param_00.var_9A2D))//titleLabel
		{
			self.var_6813.var_00E5 = param_00.var_9A2D;//label = titleLable
		}
		else
		{
		...
		if(isdefined(param_00.var_9A2D/*titleLable*/) && !isdefined(param_00.var_9A2C/*titleIsString*/))
		{
	...
	if(isdefined(param_00.var_5019))//iconOverlay
	{
		self.var_680A maps\mp\gametypes\_hud_util::func_86EF(var_04);
		if((level.var_910F || self issplitscreenplayer()) && !function_03BA())
		{
			self.var_680A setshader(param_00.var_5019,30,30);//sets icon
		}
		else
		{
			self.var_680A setshader(param_00.var_5019,60,60);//sets icon
		}
...
//Function Id: 0x265F
//Function Number: 11
//killstreakSplashNotify( splashRef, streakVal )
func_265F(param_00,param_01)
{
	...
	var_02.var_8CA3 = 0;
	thread func_0884(var_02);//actionNotify(actionData);
}
